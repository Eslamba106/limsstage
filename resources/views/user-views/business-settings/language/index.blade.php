                            ...
                            @foreach(json_decode($language['value'],true) as $key =>$data)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <form action="{{ route('language.update-status') }}" method="GET" id="language_id_{{$data['id']}}_form" class="language_id_form">
                                                @csrf
                                    ...
                                    <td>
                                        ...
                                        <form action="{{route('language.update-default-status', ['code'=>$data['code']])}}" method="GET" id="language_default_id_{{$data['id']}}_form" class="language_default_id_form">
                                            @csrf
                                            <input type="hidden" name="code" value="{{$data['code']}}">
                                            <label class="switcher mx-auto">
                                ...
                                <div class="col-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control country-var-select w-100" name="code">
                                            @foreach(\Illuminate\Support\Facades\File::files(base_path( main_path().'assets/images/flags')) as $path)
                                                @if(pathinfo($path)['filename'] !='en')
                                                    <option value="{{ pathinfo($path)['filename'] }}"
                                                            title="{{ asset(main_path().'assets/images/flags/'.pathinfo($path)['filename'].'.png') }}">
...
@section('js')
    <!-- Page level plugins -->
    <script src="{{asset(main_path().'js/jquery.dataTables.min.js')}} "></script>
    ...
    <script>
        ...
        $('.language_id_form').on('submit', function(event){
            ...
            $.ajax({
                url: "{{ route('language.update-status') }}",
                method: 'GET',
                data: $(this).serialize(),
