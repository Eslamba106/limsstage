...
@section('title')
    {{ translate('edit_report_generation_setting') }}
@endsection
@section('css')
...
@section('content')
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-5 align-self-center">
                <h4 class="page-title">{{ translate('edit_report_generation_setting') }}</h4>
            </div>
    ...
    <div class="container-fluid">
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('report_generation_setting.update', $report_generation_setting->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control"
                               value="{{ old('name', $report_generation_setting->name) }}" required>
                    </div>
                    ...
                    <div class="row mb-4">
                            ...
                            <select class="form-select" name="frequency_id" required>
                                @foreach ($frequencies as $frequency_item)
                                    <option value="{{ $frequency_item->id }}"
                                        {{ $report_generation_setting->frequency_id == $frequency_item->id ? 'selected' : '' }}>
                                        {{ $frequency_item->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="time" name="execution_time" class="form-control"
                                   value="{{ old('execution_time', $report_generation_setting->execution_time) }}" required>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" value="1" name="generation_condition"
                                   id="cond1" {{ $report_generation_setting->generation_condition == 1 ? 'checked' : '' }} required>
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" value="2" name="generation_condition"
                                   id="cond2" {{ $report_generation_setting->generation_condition == 2 ? 'checked' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" value="1" name="report_type"
                                   id="type1" {{ $report_generation_setting->report_type == 1 ? 'checked' : '' }} required>
                            <label class="form-check-label" for="type1">
                                {{ translate('All_Results') }}
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" value="2" name="report_type"
                                   id="type2" {{ $report_generation_setting->report_type == 2 ? 'checked' : '' }}>
                                    ...
                                    <select id="sample_points" name="sample_points[]"
                                        ...
                                        @foreach ($samples as $sample_item)
                                            <option value="{{ $sample_item->id }}"
                                                {{ $report_generation_setting->samples->contains($sample_item->id) ? 'selected' : '' }}>
                    ...
                    <div class="col-md-12 mb-4">
                        <div class="form-group">
                            ...
                            <select name="emails[]" class="form-select js-select2-custom form-control" multiple required>
                                @foreach ($emails as $email_item)
                                    <option value="{{ $email_item->id }}"
                                        {{ $report_generation_setting->emails->contains($email_item->id) ? 'selected' : '' }}>
                                        {{ $email_item->email }}
                                    </option>
                                @endforeach
                            </select>
