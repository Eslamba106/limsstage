    ...
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-5 align-self-center">
                <h4 class="page-title">{{ translate('clients') }}</h4>
                    ...
                    <div class="input-group mb-3 d-flex justify-content-end">
                        ...
                        {{-- @can('delete_sample')
                            <button type="submit" name="bulk_action_btn" value="delete"
                                ...
                                {{ translate('dashboard.delete') }}</button>
                        @endcan --}}
                        {{-- @can('create_sample') --}}
                            <a href="{{ route('client.create') }}" class="btn btn-secondary mt-3 mr-2">
                                ...
                                <th scope="row">
                                    <label>
                                        <input class="check_bulk_item" name="bulk_ids[]" type="checkbox"
                                            value="{{ $client_item->id }}" />
                                        <span class="text-muted">#{{ $client_item->id }}</span>
                                    </label>
                                </th>
                                <td class="text-center">{{ $client_item->name }} 
                                </td>
                                <td class="text-center">{{ $client_item->email }}</td>
                                <td class="text-center">{{ $client_item->phone }}</td>
                                

                                
                                <td class="text-center">
                                    {{-- @can('delete_sample')  --}}
                                        <a href="{{ route('client.delete', $client_item->id) }}"
                                            class="btn btn-danger btn-sm" title="@lang('dashboard.delete')"><i
                                                class="fa fa-trash"></i></a>
                                    {{-- @endcan --}}
                                    {{-- @can('edit_sample')  --}}
                                        <a href="{{ route('client.edit', $client_item->id) }}"
